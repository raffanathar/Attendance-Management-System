...
public:
    ...
    vector<string> generateAttendanceReport(double threshold) const;

    // Get attendance details for an employee
    vector<Attendance> getAttendanceDetails(const string& empId) const;

    // Helper to calculate total weekly hours for an employee
    double calculateTotalWeeklyHours(const string& empId) const;
};
